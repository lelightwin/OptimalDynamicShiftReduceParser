...
//        // create raw file
        ...
        for (Tree<String> tree : goldenTrees) {
            ...
            for (Tree<String> tokens : tree.getPreTerminals()) {
                if (!tokens.getLabel().equals("-NONE-")) { // remove empty character
        ...
        predictBFW.close();
        double avgF1 = 0.0;
        for (int i = 0; i < goldenTrees.size(); i++) {
            Tree<String> gold = goldenTrees.get(i);
            Tree<String> predict = predictTrees.get(i);

            avgF1 += eval.evaluate(predict, gold);
        }
        avgF1 /= goldenTrees.size();
        System.out.println("average F-score = " + avgF1);
    }
}
