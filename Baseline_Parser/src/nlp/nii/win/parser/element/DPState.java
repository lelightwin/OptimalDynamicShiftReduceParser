...
public class DPState implements Comparable<DPState> {
    ...
    public DPState(Sentence sentence, CustomizeStack<Tree<String>> stack, int queueSize) {
        ...
        Tree<String> st2 = stack.peek(2);
        Tree<String> st3 = stack.peek(3);

        if (st0 != null) {
            this.s0 = new DPPoint(st0, this.sentence);
            Tree<String> st0u = st0.getUnaryChild();
            Pair<Tree<String>, Tree<String>> st0lr = st0.getBinaryChild();
            ...
            if (st1 != null) {
                ...
                Tree<String> st1u = st1.getUnaryChild();
                Pair<Tree<String>, Tree<String>> st1lr = st1.getBinaryChild();
    ...
    public int s0_len() {
        if (this.hasS0()) {
            if (s0.l() < s0.f()) {
                System.out.println("s1 len failed");
            }
    ...
    public ArrayList<DPState> doBReduceWith(DPState leftState) {
        ...
        if (Conditioner.canBReduce(leftState, this)) { // check if b-reduce action could be performed
            ...
            for (Rule<Integer> rule : getBinary(left.c(), right.c())) { // process list of rules which have lhs = l and r
                if (Conditioner.canBReduceWithRule(leftState, this, rule)) { // check if b-reduce action could be performed with this resulting node (indicated by rule)
                    ...
                    if (rule.getHead() == 0) { // head is left
                        newPoint = new DPPoint(c, left.h(), left.f(), right.l());
                    } else { // head is right
                        newPoint = new DPPoint(c, right.h(), left.f(), right.l());
                    }
                    ...
                    newState.action = rule.getAction();
    ...
    public ArrayList<DPState> doUReduce() {
        ...
        if (Conditioner.canUReduce(this)) { // check if u-reduce action could be performed
            ...
            for (Rule<Integer> rule : getUnary(unary.c())) { // process list of rules which have lhs = u
                DPPoint newPoint = new DPPoint(rule.getCons(), unary.h(), unary.f(), unary.l());
    ...
    public Tree<String> makeTree() {
        ...
        if (backPointer.isEmpty()) { // this is a leaf state
            ...
            t = new Tree(DataInfo.instance().labelInStr(s0.c()));
    ...
    public String toString() {
        ...
        result += ", action=" + DataInfo.instance().actionInStr(action) + '}';
        return result;
    }
     ...
     */
    public ArrayList<DPState> getBackPointer() {
        return backPointer;
    }
     ...
     */
    public ArrayList<DPState> getLeftStates() {
        return leftStates;
    }
