...
import nlp.nii.win.corpus.stanford.util.MapFactory;
import nlp.nii.win.corpus.stanford.util.MyMethod;
...
public class Tree<L> implements Serializable, Comparable<Tree<L>>,
    ...
    public void toIntStringBuilder(StringBuilder sb) {
        if (!isLeaf()) {
            sb.append('(');
        }
        if (getLabel() != null) {
            if (!this.isLeaf()) {
                sb.append(getNumberLabel());
            } else {
                sb.append(getLabel());
            }
        }
    ...
    /**
     * @param headIdx the headIdx to set
     */
    ...
    private void removeUnaryChainHelper(Tree<L> tree, Tree<L> parent) {
        ...
        if (tree.isUnary() && !tree.isPreTerminal()) {
            if (parent != null) {
                tree = tree.getChild(0);
                parent.getChildren().set(0, tree);
                removeUnaryChainHelper(tree, parent);
            } else {
                removeUnaryChainHelper(tree.getChild(0), tree);
            }
        } else {
            for (Tree<L> child : tree.getChildren()) {
                if (!child.isPreTerminal()) {
    ...
    /**
     * only for Tree<String>
     */
    public void mergeUnaryChains() {
        mergeUnaryChainHelper((Tree<String>) this, null);
    }

    private void mergeUnaryChainHelper(Tree<String> tree, Tree<String> parent) {
        if (tree.isLeaf()) {
            return;
        }
        if (tree.isUnary() && !tree.isPreTerminal()) {
            if (parent != null) {
                parent.setLabel(parent.getLabel() + "_" + tree.getLabel());
                tree = tree.getChild(0);
                parent.getChildren().set(0, tree);
                mergeUnaryChainHelper(tree, parent);
            } else {
                mergeUnaryChainHelper(tree.getChild(0), tree);
            }
        } else {
            for (Tree<String> child : tree.getChildren()) {
                if (!child.isPreTerminal()) {
                    mergeUnaryChainHelper(child, null);
                }
            }
        }
    }
    ...
    /*--------------------------------------------------------------------------------------------------*/
    /*my custom code*/
    private boolean headToken = false; // whether this node is a head node
    ...
    public void extractHeadIdx() {
        if (!this.isPreTerminal()) {
            for (Tree<L> c : this.getChildren()) {
                c.extractHeadIdx();
                if (c.isHeadToken()) {
                    headIdx = c.getHeadIdx();
                }
            }
            this.startIdx = this.getChild(0).getStartIdx();
            this.endIdx = this.getChild(this.getChildren().size() - 1).getEndIdx();
        }
    }

    /**
     *
     * @return true if there is a unary rule (X->X) existed in tree
     */
    public boolean hasDuplicateUnary() {
        boolean result = false;
        if (this.isPreTerminal()) {
            return false;
        }
        if (this.getChildren().size() == 1) {
            if (this.getChild(0).getLabel().equals(this.getLabel())) { // has the same label as its unary child
                result = true;
            }
        }
        for (Tree<L> child : this.getChildren()) {
            result = result || child.hasDuplicateUnary(); // check recursively
        }
        return result;
    }

    /**
     * remove the redundant unary rules (X -> X)
     */
    public void removeDuplicateUnary() {
        if (this.isPreTerminal()) {
            return;
        }
        if (this.getChildren().size() == 1) { // this current node is an unary and non-terminal node
            if (this.getChild(0).getLabel().equals(this.getLabel())) { // has the same label as its unary child
                List<Tree<L>> newChildren = this.getChild(0).getChildren();
                this.setChildren(newChildren);
            }
        }

        for (Tree<L> child : this.getChildren()) {
            child.removeDuplicateUnary(); // remove recursively
        }
    }
    ...
    /**
     * @return the headToken
     */
    public boolean isHeadToken() {
        return headToken;
    }

    /**
     * @param headToken the headToken to set
     */
    public void setHeadToken(boolean headToken) {
     ...
     */
    public void setNumberLabel(int intLabel) {
    ...
    /**
     *
     * @return true if current node is a unary node
     */
    ...
    /**
     *
     * @return the unary child of this tree
     */
    public Tree<L> getUnaryChild() {
        if (this.isUnary()) {
    ...
    /**
     *
     * @return the binary children of this tree
     */
    public Pair<Tree<L>, Tree<L>> getBinaryChild() {
        if (this.isBinary()) { // if tree is binary, return its children
            return new Pair(this.getChild(0), this.getChild(1));
        } else { // if tree is unary, check its unary child
            Tree<L> uChild = this.getChild(0);
            if (uChild.isBinary()) {
                return new Pair(uChild.getChild(0), uChild.getChild(1));
            } else {
                return null;
            }
        }
    ...
    /**
     * display tree in a hierarchical way
     *
     * @param i is the tab before the label
     */
    public void display(int i) {
        if (!this.isLeaf()) {
            ...
            if (!isPreTerminal()) {
                System.out.println(getLabel() + "(" + getHeadIdx() + ")");
            } else {
                System.out.println(getLabel() + "(" + getHeadIdx() + ")" + " - " + this.getChild(0).getLabel());
            }
            for (Tree<L> c : this.getChildren()) {
                c.display(i + 1);
            }
