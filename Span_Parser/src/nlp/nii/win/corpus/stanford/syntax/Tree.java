...
public class Tree<L> implements Serializable, Comparable<Tree<L>>,
    ...
    private int start = -1; // start index of span

    private int end = -1; // end index of span
    ...
    public void extractSpanInfo() {
        if (this.isPreTerminal()) {
            this.start = this.headIdx;
            this.end = this.headIdx;
        } else {
            for (Tree<L> c : this.getChildren()) {
                c.extractSpanInfo();
            }
            this.start = this.getChild(0).getStart();
            this.end = this.getChild(this.getChildren().size() - 1).getEnd();
        }
    }
    ...
    public void display(int i) {
        if (!this.isLeaf()) {
            ...
            if (!isPreTerminal()) {
                System.out.println(getLabel() + "(" + getStart() + "," + getEnd() + ")");
            } else {
